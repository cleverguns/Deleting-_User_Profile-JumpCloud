[CmdletBinding()]
param (
    # WhatIf parameter to show actions without executing them
    [switch]$WhatIf
)
...
if ($ProfilesToDelete.Count -gt 0) {
    ...
    Write-Output ""
    # Delete user profiles if not in WhatIf mode
    if (-not $WhatIf) {
        ...
        Write-Output "Use -WhatIf parameter to execute the deletion."
    }
} else {
